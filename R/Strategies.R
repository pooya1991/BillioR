Strategies <- function (x,n,m) 
{
  ...
  if("send" %in% Mtype && "beforsend" %in% Mtype && Lims[[1]] == 1){
    ...
    if(tail(BUY,n=1)){
      ...
      Bact <- which(BUY)
      ddd <- index(tail(Bact,n=1))
      if(HLC[ddd,3] * (1 + LimP/100) < tail(HLC[,1],n=1) || HLC[ddd,3] * (1 - LimL/100) > tail(HLC[,2],n=1)){
        Act <- "S"
      }else{
        Act <- "N"
      }
  ...
  nat <- list(Action,Act)
  names(nat) <- c("NeedAction","ActionType")
  nat
}
