PranaBehgozin <- function(Stg,Share,Timeframe = "hourly",StartDate = "2014-01-01",EndDate = Sys.Date(),Vol = 1000,MaxPos = 10,Fee = T,Over = T,ReEnterType = 0,ReEnterAmm = 0, SuccessRate = 50, TotalReturn = 30, TickerID){
  ...
  Report <- function(Result,dd,MaxPos, SuccessRateRe = SuccessRate, TotalReturnRe = TotalReturn, TickerIDRe = TickerID){
    n <- nrow(Result) / 2
    ...
    validation <- FALSE
    if(TotalRet > (1 + (TotalReturnRe / 100)) & SuccessRatedu > SuccessRateRe){
      validation <- TRUE
    }
